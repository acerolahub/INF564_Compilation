			...
			if(temp instanceof Econst) {
				...
				if(n.e1 instanceof Econst && n.e2 instanceof Eaccess_local) {
					...
					if(n.b!=Binop.Band && n.b!=Binop.Bor) {
						...
						else if(n.b == Binop.Bdiv || n.b == Binop.Bsub) {
							r2 = fun_locals.get(((Eaccess_local)n.e2).i);
							stack_reg.push(rd);
							this.visit(n.e1);
							Label L6 = new Label();
							Label L7 = new Label();
				...
				else if(n.e1 instanceof Eaccess_local && n.e2 instanceof Eaccess_local) {
					Register r2 = fun_locals.get(((Eaccess_local)n.e1).i);
					Register r5 = fun_locals.get(((Eaccess_local)n.e2).i);
					if(n.b!=Binop.Band && n.b!=Binop.Bor) {
						Register rd = stack_reg.pop();
						Label Ld = stack_lab.pop();
...
public void visit(Eassign_local n) {
		...
		public void visit(Eassign_field n) {
			...
			if(tmp.e instanceof Eaccess_local) {
				...
				Label L4 = new Label();
				current_graph.graph.put(L1 ,new Rstore(rtemp2, rtemp, Memory.word_size*i, L4));
...
public void visit(Ecall n) {
		...
		public Label RTLc(Expr e, Label Lt, Label Lf) {
			...
			if(debug) System.out.println("last" +last_result);
			...
			if(debug) System.out.println(new Rmubranch(m, r, Lf, Lt));
			//System.out.println(new Rmubranch(m, r, Lt, Lf));
			if(debug)  System.out.println("last" +last_result);
			if(debug) System.out.println("RTLC fin "+L2+new Rmubranch(m, r, Lf, Lt)); 
			return L2; 
		}
		...
		public void visit(Sif n) {
			...
			this.visit(n.s2);
			if(debug)  System.out.println("Lf last" +last_result);
			Lf = last_result; 
			stack_lab = stack_lab_tmp;
			stack_reg = stack_reg_tmp;
			
			this.visit(n.s1);
			if(debug)  System.out.println("Lt last" +last_result);
			Lt = last_result; 
