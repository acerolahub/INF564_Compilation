...
class ToERTL extends EmptyRTLVisitor {
  ...
  public void visit(RTLfun d) {
    ...
    for(Label l: potential_rec) {
      ...
      if (ercall.s.equals(d.name)) {
        ...
        //   System.out.println("Appell recu term");
  ...
  public void visit(RTL rtl) {
    if (rtl instanceof Rconst) this.visit((Rconst) rtl); else if (
      rtl instanceof Rload
    ) this.visit((Rload) rtl); else if (rtl instanceof Rstore) this.visit(
        (Rstore) rtl
      ); else if (rtl instanceof Rmunop) this.visit((Rmunop) rtl); else if (
      rtl instanceof Rmbinop
    ) this.visit((Rmbinop) rtl); else if (rtl instanceof Rmubranch) this.visit(
        (Rmubranch) rtl
      ); else if (rtl instanceof Rmbbranch) this.visit(
        (Rmbbranch) rtl
      ); else if (rtl instanceof Rgoto) this.visit((Rgoto) rtl); else if (
      rtl instanceof Rcall
  ...
  public void visit(Rcall rtl) {
    ...
    if (n > 6) {
      ...
      current_graph.put(
        temp,
        new ERmunop(new Maddi((n - 6) * Memory.word_size), Register.rsp, rtl.l)
      );
    }
