...
public class Typing implements Pvisitor {
  ...
  private static final int IDENT_DECL = 0, IDENT_SRETURN = 2, IDENT_SIF =
    3, IDENT_PEVAL = 4, IDENT_SWHILE = 5;
  private int ident_block = 0;

  private static final int IDENT_EUNOP = 5, IDENT_EBINOP = 6, IDENT_ECALL =
    7, IDENT_PASSIGN = 10;

  private static final int STRUCT_NAME = 1, STRUCT_VARNAME = 2, STRUCT_FIELD =
    3;

  private String NAME_FIELD;
  private Typ TYP_FIELD;
  private String ORIGIN_TYP;
    ...
    if (stack_struct.peek() == 0) {
      if (cursor == 0 && stack_struct.peek() == 0) {
        for (Decl_fun funs : file.funs) {
          ...
          ) throw new Error(String.format("line %d, column %d:\nredefinition of function \"%s\"", n.loc.line, n.loc.column, n.id));
        }
        file.funs.getLast().fun_name = n.id;
        if (n.id.equals("main")) nb_main++;
        if (nb_main > 1) throw new Error(String.format("More than one function named main (%d)", nb_main));
      } else if (cursor <= nb_args && stack_struct.peek() == 0) {
        for (Decl_var var : file.funs.getLast().fun_formals) {
          if (var.name != null && var.name.equals(n.id)) throw new Error(String.format("line %d, column %d:\nredefinition of variable \"%s\"", n.loc.line, n.loc.column, n.id)
        ...
        switch (ident_block) {
          ...
          default:
            ...
            if (present == 0) throw new Error("unknown variable at " + n.loc);
      ...
      if (structure.get(name_struct).fields.containsKey(n.id)) throw new Error(
        "redefinition of variable at location " + n.loc
      );
    ...
    if (stack_struct.peek() == 0) {
      if (cursor == 0) {
        ...
        if (s == null) throw new Error(
          "No such struct " + n.loc
        ); else file.funs.add(new Decl_fun(new Tstructp(s), null, null, null));
      } else if (cursor <= nb_args) {
        s = structure.get(n.id);
        if (s == null) throw new Error(
          "No such struct " + n.loc
      ...
      if (structure.get(n.id) == null) throw new Error(
        "structure not defined at location " + n.loc
      );
      type_field = n.id;
    }
  ...
  public void visit(Passign n) {
    ...
    if (n.e1 instanceof Parrow) {
      ...
      if (t1 instanceof Tint && t2 instanceof Tstructp) throw new Error(
        "incompatible type operation at location " + n.loc
      );
      if (t1 instanceof Tstructp && t2 instanceof Tstructp) {
        if (
          ((Tstructp) t1).s.str_name != ((Tstructp) t2).s.str_name
        ) throw new Error("incompatible type operation at location " + n.loc);
      }
  ...
  public void visit(Parrow n) {
    ...
    if (f instanceof Eaccess_local) {
        ...
        for (Decl_var var : blc.dl) {
          if (
            ...
            if (
              ...
              "this structure don't have a such field location: " + n.loc
            );
            TYP_FIELD =
              structure
      ...
      if (present == 0) throw new Error("Not a structure at location " + n.loc);
    ...
    } else throw new Error("Not a structure at location " + n.loc);
  ...
  public void visit(Pcall n) {
    ...
    ); else if (cmp == file.funs.size() && n.l.size() != 1) throw new Error(
      String.format(
        "line %d, column %d:\nbad arguments number",
        n.loc.line,
        n.loc.column
  ...
  public void visit(Psizeof n) {
    ...
    if (structure.get(n.id) == null) throw new Error(
      "Psizeof: structure not found at location " + n.loc
    );
  ...
  public void visit(Pbloc n) {
    ...
    int rreturn = 0;
    for (Pstmt tmp : n.sl) {
      if (FIN_FCT == 1) throw new Error("code not reachable " + tmp.loc);
      if (rreturn == 0) {
  ...
  public void visit(Pfun n) {
    ...
    if (
      ...
      if (FIN_FCT == 0) throw new Error(
        "return of function " +
        file.funs.getLast().fun_name +
