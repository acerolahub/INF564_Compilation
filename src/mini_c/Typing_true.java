...
public class Typing implements Pvisitor {
		...
		public void visit(Pident n) {
			...
			if(stack_struct.peek()==0) {
				...
				if(type_field.equals("0")) 
					structure.get(name_struct).fields.put(n.id, new Field(n.id, new Tint(), current_size));
				else
					structure.get(name_struct).fields.put(n.id, new Field(n.id, new Tstructp(structure.get(type_field)), current_size));
	...
	public void visit(Passign n) {
		...
		if(n.e1 instanceof Parrow) {
			...
			if (TYP_FIELD instanceof Tint) {
				...
				((Eassign_field)e).f = new Field(NAME_FIELD, new Tstructp(structure.get(((Tstructp)TYP_FIELD).s.str_name)), -1);
			}
			
			((Eassign_field)e).f.pos = structure.get(ORIGIN_TYP).fields.get(NAME_FIELD).pos;
			...
			if(t2 instanceof Tint && t1 instanceof Tstructp) {
				if(!(((Eassign_field)e).e2 instanceof Econst && ((Econst)((Eassign_field)e).e2).i==0))
					throw new Error("incompatible type operation at location " + n.loc);
			}
	...
	public void visit(Parrow n) {
		...
		if(f instanceof Eaccess_local) {
			...
			Typ T2 = TYP_FIELD;
			TYP_FIELD = structure.get(((Tstructp)T2).s.str_name).fields.get(n.f).field_typ;
			NAME_FIELD = n.f;
