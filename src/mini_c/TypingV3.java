...
public class Typing implements Pvisitor {
		...
		if(cursor==0)
			...
			case IDENT_SIF: 
				...
				case IDENT_EACCES_LOCAL: 
					...
					((Sif)((Sblock)file.funs.getLast().fun_body).sl.getLast()).e = new Eaccess_local(n.id); 
					break; 
				case IDENT_EUNOP:
				case IDENT_EBINOP: 
					//si première expression du binop
	...
	public void visit(Pif n) {
		...
		stack_addr_expr.push(((Sif)((Sblock)file.funs.getLast().fun_body).sl.getLast()).e); 
		this.visit(tmp);
		((Sif)((Sblock)file.funs.getLast().fun_body).sl.getLast()).e = stack_addr_expr.pop() ; 
		
		//System.out.println(n.s1 instanceof Pstmt); 
	...
	public void visit(Preturn n) {
		...
		this.visit(tmp);
		((Sreturn)((Sblock)file.funs.getLast().fun_body).sl.getLast()).e =stack_addr_expr.pop() ; 
